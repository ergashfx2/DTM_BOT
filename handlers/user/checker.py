...
async def callback_fun(call):
    ...
    try:
        if call.data == "check_subs":
            ...
            if not final_status:
                await call.message.answer(Text_caption[0], reply_markup=types.InlineKeyboardMarkup(inline_keyboard=chs), disable_web_page_preview=True, parse_mode="markdown")
            else:
